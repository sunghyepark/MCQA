...
void QASMparser::check(Token::Kind expected) {
    if (sym == expected) {
        ...
        std::cerr << "ERROR while parsing QASM file: expected '" << Token::KindNames[expected] 
                  << "' but found '" << Token::KindNames[sym] 
                  << "' in line " << la.line << ", column " << la.col << std::endl;
    }
}
...
QASMparser::Expr* QASMparser::QASMexponentiation() {
    ...
    if(sym == Token::Kind::real) {
        ...
        scan();
        return new Expr(Expr::Kind::number, NULL, NULL, QASM_PI, "");
...
#elif GATESET_QUTECH
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
void QASMparser::addRXgate(int target, double theta) {
    Qcircuit::Gate g;

    g.id = ngates;
    g.target = target;
    g.control = -1;
    g.angle = theta; // for circuit_optimization.cpp

    logi_qubits[target] = true;

    g.type = GATETYPE::RX;
    snprintf ( g.output_type, 127, "(%.2f)", theta);

    gatelistsL.push_back(g);
    
    ngates++;
}
void QASMparser::addRYgate(int target, double theta) {
    Qcircuit::Gate g;
...
void QASMparser::addCZgate(int target, int control) {
    Qcircuit::Gate g;
...
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
    if(sym == Token::Kind::rxgate) {
        ...
        if(execute) {
            for(int i = 0; i < target.second; i++) {
                addRXgate(target.first+i, theta->num);
            }
        }
        delete theta;
        ...
        if(execute) {
            for(int i = 0; i < target.second; i++) {
                addRYgate(target.first+i, theta->num);
            }
        }
        delete theta;
        ...
        if(execute) {
            if(control.second == target.second) {
                for(int i = 0; i < target.second; i++) {
                    addCZgate(target.first+i, control.first+i);
                }
            } else if(control.second == 1) {
                for(int i = 0; i < target.second; i++) {
                    addCZgate(target.first+i, control.first);
                }
            } else if(target.second == 1) {
                for(int i = 0; i < target.second; i++) {
                    addCZgate(target.first, control.first+i);
                }
            } else {
                std::cerr << "Register size does not match for CZ gate!" << std::endl;
            }
#if VERBOSE
            std::cout << "Applied gate: CX" << std::endl;
#endif
...
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
                    if(RXgate* rx = dynamic_cast<RXgate*>(*it)) {
                        Expr* theta = RewriteExpr(rx->theta, paramsMap);
                        for(int i = 0; i < argsMap[rx->target].second; i++) {
                            addRXgate(argsMap[rx->target].first+i, theta->num);
                        }
                        delete theta;

                    } else if(RYgate* ry = dynamic_cast<RYgate*>(*it)) {
                        Expr* theta = RewriteExpr(ry->theta, paramsMap);
                        for(int i = 0; i < argsMap[ry->target].second; i++) {
                            addRYgate(argsMap[ry->target].first+i, theta->num);
                        }
                        delete theta;

                    } else if(CZgate* cz = dynamic_cast<CZgate*>(*it)) {
                        if(argsMap[cz->control].second == argsMap[cz->target].second) {
                            for(int i = 0; i < argsMap[cz->target].second; i++) {
                                addCZgate(argsMap[cz->target].first+i, argsMap[cz->control].first+i);
                            }
                        } else if(argsMap[cz->control].second == 1) {
                            for(int i = 0; i < argsMap[cz->target].second; i++) {
                                addCZgate(argsMap[cz->target].first+i, argsMap[cz->control].first);
                            }
                        } else if(argsMap[cz->target].second == 1) {
                            for(int i = 0; i < argsMap[cz->target].second; i++) {
                                addCZgate(argsMap[cz->target].first, argsMap[cz->control].first+i);
                            }
                        } else {
                            std::cerr << "Register size does not match for CZ gate!" << std::endl;
                        }
...
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
        if(sym == Token::Kind::rxgate) {
            ...
            check(Token::Kind::rpar);
            check(Token::Kind::identifier);
            gate.gates.push_back(new RXgate(theta, t.str));
            check(Token::Kind::semicolon);
            ...
            check(Token::Kind::rpar);
            check(Token::Kind::identifier);
            gate.gates.push_back(new RYgate(theta, t.str));
            check(Token::Kind::semicolon);
            ...
            check(Token::Kind::comma);
            check(Token::Kind::identifier);
            gate.gates.push_back(new CZgate(control, t.str));
            check(Token::Kind::semicolon);
        }
// <<<<<<<<<<<<<<<<<<<< QuTech gateset <<<<<<<<<<<<<<<<<<<<
#endif // GATESET
...
// >>>>>>>>>>>>>>>>>>>> IIC-JKU Original gateset >>>>>>>>>>>>>>>>>>>>
                if(Ugate* u = dynamic_cast<Ugate*>(*it)) {
                    gate.gates.push_back(new Ugate(RewriteExpr(u->theta, paramsMap), 
                                                   RewriteExpr(u->phi, paramsMap), 
                                                   RewriteExpr(u->lambda, paramsMap), 
                                                   argsMap[u->target]));
...
// >>>>>>>>>>>>>>>>>>>> DAC2022 target gateset >>>>>>>>>>>>>>>>>>>>
                if(Xgate* x = dynamic_cast<Xgate*>(*it)) {
                    ...
                    gate.gates.push_back(new RXgate(RewriteExpr(rx->theta, paramsMap), 
                                                    RewriteExpr(rx->phi, paramsMap), 
                                                    RewriteExpr(rx->lambda, paramsMap), 
                                                    argsMap[rx->target]));

                } else if(RZgate* rz = dynamic_cast<RZgate*>(*it)) {
                    gate.gates.push_back(new RZgate(RewriteExpr(rz->theta, paramsMap), 
                                                    RewriteExpr(rz->phi, paramsMap), 
                                                    RewriteExpr(rz->lambda, paramsMap), 
                                                    argsMap[rz->target]));
...
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
                if(RXgate* rx = dynamic_cast<RXgate*>(*it)) {
                    ...
                    gate.gates.push_back(new RYgate(RewriteExpr(ry->theta, paramsMap), 
                                                    argsMap[ry->target]));

                } else if(CZgate* cz = dynamic_cast<CZgate*>(*it)) {
                    gate.gates.push_back(new CZgate(argsMap[cz->control], argsMap[cz->target]));
                }
// <<<<<<<<<<<<<<<<<<<< QuTech gateset <<<<<<<<<<<<<<<<<<<<
#endif // GATESET
                else {
...
#if GATESET_IIC_JKU
// >>>>>>>>>>>>>>>>>>>> IIC-JKU Original gateset >>>>>>>>>>>>>>>>>>>>
        sym == Token::Kind::ugate || 
        sym == Token::Kind::cxgate ||
// <<<<<<<<<<<<<<<<<<<< IIC-JKU Original gateset <<<<<<<<<<<<<<<<<<<<
#elif GATESET_DAC2022
// >>>>>>>>>>>>>>>>>>>> DAC2022 target gateset >>>>>>>>>>>>>>>>>>>>
        ...
        sym == Token::Kind::rxgate ||
        sym == Token::Kind::rzgate ||
        sym == Token::Kind::cxgate ||
// <<<<<<<<<<<<<<<<<<<< DAC2022 target gateset <<<<<<<<<<<<<<<<<<<<
#elif GATESET_QUTECH
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
        sym == Token::Kind::rxgate ||
        sym == Token::Kind::rygate ||
        sym == Token::Kind::czgate ||
// <<<<<<<<<<<<<<<<<<<< QuTech gateset <<<<<<<<<<<<<<<<<<<<
#endif // GATESET 
        sym == Token::Kind::identifier) {
        QASMgate(execute);
...
#endif // GATESET
                sym == Token::Kind::identifier || 
                sym == Token::Kind::measure || 
                sym == Token::Kind::reset) {
            QASMqop();
