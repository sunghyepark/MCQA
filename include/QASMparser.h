...
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
    ...
    public:
        ...
        RXgate(Expr* theta, std::string target) {
            this->theta = theta;
            this->target = target;
        }
    ...
    class RYgate : public BasisGate {
    public:
        Expr* theta;
        std::string target;

        RYgate(Expr* theta, std::string target) {
            this->theta = theta;
            this->target = target;
        }
    ...
    class CZgate : public BasisGate {
    public:
        std::string control;
        std::string target;

        CZgate(std::string control, std::string target) {
            this->control = control;
            this->target = target;
        }
    };
...
#elif GATESET_QUTECH
// >>>>>>>>>>>>>>>>>>>> QuTech gateset >>>>>>>>>>>>>>>>>>>>
    void addRXgate(int target, double theta);
    void addRYgate(int target, double theta);
    void addCZgate(int target, int control);
// <<<<<<<<<<<<<<<<<<<< QuTech gateset <<<<<<<<<<<<<<<<<<<<
#endif // GATESET    
};
